...
async def add_job(image: str):
    ...
    for container in client.containers.list(all=True):
        ...
        if container_image == image:
            ...
            if container.status == "running":
                ...
                port = (
                    port_mappings["5000/tcp"][0]["HostPort"]
                    if "5000/tcp" in port_mappings
                    else None
                )
                break
    ...
    if existing_container:
        ...
        if any(job["status"] == "running" for job in jobs.values()):
            raise HTTPException(
                status_code=400, detail="Another job is currently running."
            )
    ...
    Thread(
        target=health_check_routine, args=(job_id, container.id, port), daemon=True
    ).start()
    ...
    if job["status"] == "predicting":
        ...
        try:
            ...
            response = requests.post(
                f"http://{ssh_host}:{port}/predictions", json={"input": input}
            )
            if response.status_code == 200:
